...
function MoviesCardList(props) {
    ...
    const renderFilms = () => {
        ...
        return ((props.isSubmitted && props.view.length === 0) ? <p className='movies__empty'>Ничего не найдено...</p> : chunk.map((movie) => (

        <MoviesCard saved={props.saved} user={props.user} onDelClick={props.onDelClick} onSaveClick={props.onSaveClick} movie={movie} key={movie.id} name={movie.nameRU} duration={movie.duration} trailerLink={movie.trailerLink} link={`${baseUrl + movie.image.url}`} />)))
